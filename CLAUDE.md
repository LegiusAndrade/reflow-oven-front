...
The project is at an early scaffolding stage — `src/app/page.tsx` is still a placeholder. UI text and metadata are in **Portuguese (pt-BR)**.
...
- Semicolons always required; 2-space indent; 100-char print width; ES5 trailing commas.
...
The Roboto font is wired up via `next/font` in `src/app/layout.tsx` and exposed as the `--font-roboto` CSS variable, consumed in `globals.css`.
