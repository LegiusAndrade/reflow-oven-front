...
- **Scroll gutter:** a vertical scroll area (`overflow-y-auto`) must reserve the scrollbar lane so the bar never sits on top of the content (overlay scrollbars) nor shifts/clips it (classic). Add **`[scrollbar-gutter:stable]`** to the scroller — not `pr-*` padding, which under-reserves vs. an overlay bar. For a **rounded, bordered scroll box** (e.g. tables) the bar also collides with the rounded corners, so wrap it: border+radius on an `overflow-hidden` outer element, scroll + `scrollbar-gutter:stable` on the inner child — use the shared **`TableScrollBox`** component (never put `overflow-auto` directly on the rounded+border element). The device's browser uses overlay scrollbars (0 width, painted over content); headless Chromium does too and `::-webkit-scrollbar` CSS won't force a classic one — so none of this shows up in screenshots. Verify deliberately by reasoning + DOM measurement: `scrollbar-gutter:stable` makes `offsetWidth − clientWidth` jump to ~15px (the reserved lane).
