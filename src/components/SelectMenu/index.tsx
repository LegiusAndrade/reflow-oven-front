  ...
  return (
    <div ref={ref} className={clsx("relative", className)}>
      <button
        ...
        aria-expanded={open}
        className='btn-press flex w-full items-center justify-between gap-2 rounded-xl border border-(--border) px-4 py-2.5'
      >
      ...
      {open && (
        <ul role='listbox' className='card absolute right-0 z-30 mt-2 max-h-72 w-full min-w-max overflow-auto rounded-xl border border-(--border) py-1'>
          {options.map((option) => {
            ...
            return (
              <li key={option.value}>
                <button
                  ...
                  className={clsx("flex w-full cursor-pointer items-center gap-2 px-4 py-2.5 text-left hover:bg-(--hover)", isSelected && "text-(--brand)")}
                >
