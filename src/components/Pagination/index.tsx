        ...
        item === "ellipsis" ? (
          ...
          <button
            ...
            className={clsx(
              ...
              item === active ? "btn-link-active" : "hover:bg-(--hover)"
            )}
          >
            {item + 1}
          </button>
...
function PagerArrow({ icon, label, onClick, disabled }: { icon: string; label: string; onClick: () => void; disabled: boolean }) {
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className='btn-press grid size-9 cursor-pointer place-items-center rounded-lg hover:bg-(--hover) disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent'
    >
