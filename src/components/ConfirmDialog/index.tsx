  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-modal='true'
        aria-label={title}
        className={clsx("card relative z-10 w-full max-w-md rounded-2xl border border-(--border) p-6 transition-all duration-200", open ? "scale-100 opacity-100" : "scale-95 opacity-0")}
      >
        <div className='flex items-start gap-4'>
          <span className={clsx("grid size-12 shrink-0 place-items-center rounded-full", danger ? "bg-red-500/15 text-red-700 dark:text-red-400" : "bg-(--surface-2) text-(--brand)")}>
        ...
        <div className='mt-6 flex items-center justify-end gap-3'>
          <button type='button' onClick={onCancel} className='btn-press cursor-pointer rounded-xl border border-(--border) px-5 py-2.5 font-semibold'>
            {cancelLabel}
          </button>
          <button
            type='button'
